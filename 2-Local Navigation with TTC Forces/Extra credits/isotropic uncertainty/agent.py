    ...
    def computeForces(self, neighbors=[]):
        ...
        for j in range(len(neighbors)):
            if self.id != neighbors[j].id and not neighbors[j].atGoal:
                dis_test = sqrt((neighbors[j].pos[0]-self.pos[0])**2 + (neighbors[j].pos[1]-self.pos[1])**2) - (self.radius * 2)
                if(dis_test < dh):
                    # Calculating TTC
                    r = neighbors[j].radius + self.radius
                    w = self.pos - neighbors[j].pos #relative position
                    c = w.dot(w) - r*r
                    # Accounting for isotropic uncertainty implementation
                    rel_vel = (self.vel - neighbors[j].vel) + n
                    # Accounting for uncertainty in the sensed velocities 'e'
                    a = np.dot(rel_vel,rel_vel) - (e**2)
                    b = np.dot(rel_vel,w) - (e*r)
                    # Power Law calculation of roots
                    discr = (w.dot(rel_vel))**2 - ((rel_vel.dot(rel_vel)) * ((w.dot(w)) - r**2))
                    if b > 0:
                        ttc = float('inf')
                    if discr <= 0:
                        ttc = float('inf')
                    if discr > 0:
                        ...
                        if tau < 0:
                            ...
                            ttc = tau
                    if (ttc != float('inf')):
                        # Power law avoidance force calculation
                        Favoid = (k*math.exp(-(ttc/to))/(ttc**(m+1)))*(m + ttc/to)*((w + rel_vel*ttc)/sqrt(discr))
