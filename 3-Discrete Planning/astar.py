        ...
        if current_node == goal:
            ...
            while current_node != start:
                ...
                if parchi_diff == -3:
                    ...
                    parchi_diff = 1
                path[parent_node[0]][parent_node[1]] = action_name[action.index(parchi_diff)]
                current_node = parent_node
            break
        closed_set.add(current_node)

    # Generate neighbors
        for index,actions in enumerate(action): # Adjacent squares
            ...
            if theta == -1:
                theta = 3
            # Get node position
            neighbors = (current_node[0]+forward[theta][0], current_node[1]+forward[theta][1],theta)
            # Make sure within range
            if (0 <= neighbors[0] <= (len(grid)-1)) and (0 <= neighbors[1] <= (len(grid[len(grid)-1])-1)):
                #Make sure walkable terrain
                if (grid[neighbors[0]][neighbors[1]]) == 0:
                    if neighbors in closed_set:
                        continue
                    else:
                        g = address.g + cost[index]
                        if neighbors not in open_set:
                            h =  heuristic[neighbors[0]][neighbors[1]]
                            g = address.g + cost[index]
                            f = g+h
                            open_set.put(neighbors, Value(f=f,g=g))
                            parent[theta][neighbors[0]][neighbors[1]] = current_node[0],current_node[1],current_node[2]
                        elif neighbors in open_set and g > address.g:
                            h =  heuristic[neighbors[0]][neighbors[1]]
                            g = address.g + cost[index]
                            f = g+h
                            open_set.put(neighbors, Value(f=f,g=g))
